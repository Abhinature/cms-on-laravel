    ...
    <div class="row mb-2">
      ...
      <div class="col-sm-6">
        <ol class="breadcrumb float-sm-right">
          <li class="breadcrumb-item active">Dashboard / Downloads</li>
        </ol>
      </div>
      <!-- /.col -->
    </div>
  ...
  <div class="card mb-4">
    ...
    <div class="card-body">
      <div class="tab" id="myTab">
        <button class="tablinks" onclick="openCity(event, 'America')">Downloads</button>
                    ...
                    <table class="table table-stripped">
                      ...
                      <tbody class="text-center">
                        ...
                        <tr>
                          ...
                          <td>{{$a = $a+1}}</td>
                          <td>{{$wp->en_category}} ({{$wp->hi_category}})</td>
                          ...
                          <td>
                            ...
                            @if($wp->status == '1')
                            <button rel="{{$wp->id}}" type="button" class="btn btn-primary editdownloadmodal">Another Request</button>
                            @elseif($wp->status == '2')
                            <a href="{{route('submitforsuperadmin',['id' => Crypt::encrypt($wp->id),'tab'=>Crypt::encrypt('download')])}}" class="btn btn-success">Submit For Review</a>
                            <button rel="{{$wp->id}}" type="button" class="btn btn-primary editdownloadmodal">Edit</button>
                            <a class="btn btn-danger" href="{{route('del-content',['id' => $wp->id,'rel'=>'download'])}}">Delete</a>
                            @elseif($wp->status == '12')
                            <a href="{{route('request-for-delete-sent',['id' => Crypt::encrypt($wp->id),'tab'=>Crypt::encrypt('download_request_for_delete')])}}" class="btn btn-success">Submit For Review</a>
                            <button rel="{{$wp->id}}" type="button" class="btn btn-primary editdownloadmodal">Edit</button>
                            <a class="btn btn-danger" href="{{route('del-content',['id' => $wp->id,'rel'=>'download'])}}">Delete</a>
                            @elseif($wp->status == '0')
                            <a href="{{route('submitforsuperadmin',['id' => Crypt::encrypt($wp->id),'tab'=>Crypt::encrypt('download')])}}" class="btn btn-success">Submit For Review</a>
                            ...
                            <button type="button" rel_id="{{$wp->id}}" rel="download" class="mt-3 p-2 btn btn-success changeapprovesch">Change Approval Schedule</button>
                            <button type="button" rel_id="{{$wp->id}}" rel="download" class="mt-3 p-2 btn btn-danger cancelapproval">Cancel Approval</button>
                            @elseif($wp->status =='2')
                            Submitted For Review : {{$wp->remarks ??''}}
                            @elseif($wp->status =='11' || $wp->type =='2' )
                            <button type="button" rel_id="{{$wp->id}}" rel="delete_request_download" class="btn btn-success delete_request">Approve</button>
                            <button type="button" rel_id="{{$wp->id}}" rel="download" class="btn btn-danger reject">Reject</button>
                            <button type="button" rel_id="{{$wp->id}}" rel="download" class="btn btn-warning review">Review</button>
                            @else
                            <button type="button" rel_id="{{$wp->id}}" rel="download" class="btn btn-success approve">Approve</button>
                            <button type="button" rel_id="{{$wp->id}}" rel="download" class="btn btn-danger reject">Reject</button>
                            <button type="button" rel_id="{{$wp->id}}" rel="download" class="btn btn-warning review">Review</button>
                            @endif
                            @endif
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">Download's</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <form method="POST" enctype="multipart/form-data" action="{{route('save-download')}}">
        <div class="modal-body">
          @csrf
          <div class="form-group">
            <label>Category (In English)</label><span>*</span>
            <input class="form-control" required name="en_category" type="text">
          </div>
          <div class="form-group">
            <label>Category (In Hindi)</label><span></span>
            <input class="form-control" name="hi_category" type="text">
          </div>
          <div class="form-group">
            <label>Subject (In English)</label><span>*</span>
            <input class="form-control" name="en_subject" type="text">
          </div>
          <div class="form-group">
            <label>Subject (In hindi)</label><span></span>
            <input class="form-control" name="hi_subject" type="text">
          </div>
          <div class="form-group">
...
<script type="text/javascript">
  ...
  $('body').on('click', '.editdownloadmodal', function() {
    var pro_id = $(this).attr('rel');
    $.ajax({
      type: 'Get',
      url: "{{ url('edit-download')}}" + '/' + pro_id,
      success: function(data) {
        if (data.status == 200) {
  ...
  $('body').on('click', '.delete_request', function() {
    ...
    var type_id = $(this).attr('rel_id')
    swal({
      title: "Are you sure,You want to Approve the Delete Request?",
      icon: "warning",
      showCancelButton: true,
      confirmButtonColor: "#DD6B55",
      confirmButtonText: "Yes, archive it!",
      ...
      if (willDelete) {
        $.ajax({
          ...
          success: function(xa) {
            if (xa.success == true) {
              $('.delete_request_relation').val(rel);
              $('.delete_request_relid').val(type_id);
  ...
  $('body').on('click', '.delete_request_save', function() {
    var relation = $('.delete_request_relation').val();
    var relid = $('.delete_request_relid').val();
    var otpval = $('.delete_request_otpval').val();
    $.ajax({
      type: 'Post',
