    ...
    <div class="row mb-2">
      ...
      <div class="col-sm-6">
        <ol class="breadcrumb float-sm-right">
          <li class="breadcrumb-item active">Dashboard / E-Library</li>
        </ol>
      </div>
      <!-- /.col -->
    </div>
  ...
  <div class="card mb-4">
    ...
    <div class="card-body">
      <div class="tab" id="myTab">
        <button class="tablinks" onclick="openCity(event, 'America')">E-Library</button>
                      ...
                      <tbody class="text-center">
                        ...
                        <tr>
                          ...
                          <td>
                            ...
                            @if($wp->status == '1')
                            <button rel="{{$wp->id}}" type="button" class="btn btn-primary editlibrarymodal">Another Request</button>
                            @elseif($wp->status == '2')
                            <button rel="{{$wp->id}}" type="button" class="btn btn-primary editlibrarymodal">Edit</button>
                            <a class="btn btn-danger" href="{{route('del-content',['id' => $wp->id,'rel'=>'library'])}}">Delete</a>
                            @elseif($wp->status == '0')
                            <a href="{{route('submitforsuperadmin',['id' => Crypt::encrypt($wp->id),'tab'=>Crypt::encrypt('library')])}}" class="btn btn-success">Submit For Review</a>
                            <button rel="{{$wp->id}}" type="button" class="btn btn-primary editlibrarymodal">Edit</button>
                            <a class="btn btn-danger" href="{{route('del-content',['id' => $wp->id,'rel'=>'library'])}}">Delete</a>
                            @endif
                            ...
                            <button type="button" rel_id="{{$wp->id}}" rel="library" class="mt-3 p-2 btn btn-success changeapprovesch">Change Approval Schedule</button>
                            <button type="button" rel_id="{{$wp->id}}" rel="library" class="mt-3 p-2 btn btn-danger cancelapproval">Cancel Approval</button>
                            @elseif($wp->status =='2')
                            Submitted For Review : {{$wp->remarks ??''}}
                            @else
                            <button type="button" rel_id="{{$wp->id}}" rel="library" class="btn btn-success approve">Approve</button>
                            <button type="button" rel_id="{{$wp->id}}" rel="library" class="btn btn-danger reject">Reject</button>
                            <button type="button" rel_id="{{$wp->id}}" rel="library" class="btn btn-danger review">Review</button>
                            @endif
                            @endif
                          </td>
                        </tr>
...
<div class="modal fade" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <form method="POST" enctype="multipart/form-data" action="{{route('save-library')}}">
...
<script type="text/javascript">
  ...
  $('body').on('click', '.editlibrarymodal', function() {
    var pro_id = $(this).attr('rel');
    $.ajax({
      type: 'Get',
      url: "{{ url('edit-library')}}" + '/' + pro_id,
      success: function(data) {
        $('.editmodalbyajax1').html(data);
        $('.editmodalbyajax').modal('show');
      }
    });
  });
